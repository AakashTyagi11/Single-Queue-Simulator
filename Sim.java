...
public static double Clock, MeanInterArrivalTime, MeanServiceTime, SIGMA, LastEventTime,
        TotalBusy, MaxQueueLength, SumResponseTime,MeanArrivalCalculated, MeanServiceCalculated,Temp;
...
public static void main(String argv[]) {
 ...
 public static void Initialization()   { 
  ...
  LongService = 0;
  MeanServiceCalculated=0.0;
  MeanArrivalCalculated=0.0;

  // create first arrival event
  Event evt = new Event(arrival,MeanArrivalCalculated= exponential( stream, MeanInterArrivalTime));
 ...
 public static void ProcessArrival(Event evt) {
  ...
  Temp=exponential(stream, MeanInterArrivalTime);
  MeanArrivalCalculated=MeanArrivalCalculated+Temp;
  
  Event next_arrival = new Event(arrival, Clock+Temp);
 ...
 public static void ScheduleDeparture() {
  ...
  MeanServiceCalculated= MeanServiceCalculated+ServiceTime;
...
public static void ProcessDeparture(Event e) {
 ...
 // the departure of the next one
  ...
  double response = (Clock - finished.get_time()); // arrival time + service time -arrival time
...
MeanServiceCalculated=MeanServiceCalculated/TotalCustomers;
MeanArrivalCalculated=MeanArrivalCalculated/TotalCustomers;
...
System.out.println( "\tMeanServiceCalculated                          " +  MeanServiceCalculated);
System.out.println( "\tMeanArrivalCalculated                          " + MeanArrivalCalculated );
}
